...
**229 statically-defined alerts** across 11 domains (re-derive with
`grep -rhoE "^\s+- alert: " kubernetes/infra/configs/observability/metrics/prometheusrules/ | wc -l`
— by domain, re-counted 2026-09-07: postgres 55, microservices 52,
victoriametrics 31, kubernetes 29, observability 18, envoy-gateway 12, gitops 9,
valkey 7, keycloak 5, kyverno 4, keda 4, + the watchdog at the directory root =
227 in `.yaml`, plus the 2 in the retired `.bak` the glob also sees). The
`observability 19` this list carried was stale — that directory held 17, and
holds 18 since `ClickHouseS3Errors` (2026-09-07).
...
**237** — 227 here, minus the 2 gated CNPG topology rules that never deploy,
...
not in this number, a cluster deploys **299**.

Of those 227, **6 cannot fire on Kind** and are documented as such rather than
counted as coverage — the PVC and CNPG disk rules plus `KubeletTooManyPods`; see
§8b's "Alerts that are inert on Kind". Effective coverage is **221**. Naming that
